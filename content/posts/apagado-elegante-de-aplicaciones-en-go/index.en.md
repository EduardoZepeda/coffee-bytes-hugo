...
In Linux, these signals can be sent to an application via the [kill command](/linux-commands-you-should-know-part-two/), specifying the value of the signal and the pid of the application.
...
Remember that, in go, [operations that send or receive channel values are blocking inside their own goroutine](/go-channels-understanding-the-deadlocks/), i.e., they keep code execution on hold?
...
Finally, for this function to run correctly, we need it to run inside its [own goroutine](/go-introduction-to-goroutines-and-concurrency/). To do this, simply prefix the function call with the keyword go.
